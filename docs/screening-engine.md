...
`blue_chip_income`、`capital_heat`、`theme_momentum`、`volume_breakout` 把
`volume_ratio_min` 用作**硬过滤**，因此快照缺这一列时它们会直接失败，而不是降级运行。
...
或调小 `DAILY_ENRICH_MAX_CANDIDATES`（工作流默认 300）。

**这个上限决定了那三个策略的真实覆盖面**：日线硬筛只在 Top-N 子集上判定，其余候选连判都没判就出局。实测 `shrink_pullback` 快照层存活 3123 只，取 100 时只有 3.2% 的候选被真正判定过，取 300 时约 9.6%。子集按**快照层因子分**预排，而快照因子（估值/流动性/动量/活跃度）与「缩量回踩」「放量突破」这类形态判据基本无关，所以提高这个值是有实际意义的，不是单纯多花时间。
...
最后一步的容量裁剪**不按 `latest_score` 全桶排序**，而是各策略轮流取票。

分桶解决了跨桶不可比，但桶内不同策略之间分数同样不可比——`latest_score` 是**该策略
硬筛存活池内的分位排名**，池子大小差一个数量级时分数没有可比性。进攻桶实测：

| 策略 | 硬筛存活池 | 分数区间 |
| --- | --- | --- |
| `volume_breakout` | 19 | 70.13 ~ 82.22 |
| `capital_heat` | 93 | 68.93 ~ 73.84 |
| `theme_momentum` | 128 | 67.98 ~ **69.40** |

`theme_momentum` 的**最高分低于 `volume_breakout` 的最低分**。按分数排序时 15 个名额里
13 个是 `volume_breakout`，`theme_momentum` 仅有的 2 只还都是与其他策略共享的——
独立席位为 0。这不是它选的票差，是 19 只池子里排第 3 和 128 只池子里排第 3 拿到的
分位本就不同。

轮转规则：各策略按**自己的**分数排队，然后轮流取第 1、第 2……直到填满容量。
用同一批历史数据回放，进攻桶从 `volume_breakout` 10 / `capital_heat` 6 / `theme_momentum` 4
变成 5 / 8 / 7。

相比写死"每策略 5 个席位"，轮转的好处是：策略增删时自动适配，不必改代码；某策略候选
不足时名额自然流向其他策略而不是空置；**全程不跨策略比较分数**，只比较组内次序。

两条附加规则：

- **多策略共同命中的票优先入选**，即使它在各策略内排名靠后。三个独立策略同时背书是
  名单里最强的信号，不该因为某一策略内排名靠后被轮空。它只占一个名额，记在主策略名下。
- **轮转顺序按策略名固定**，同一份输入永远得到同一份名单，与字典顺序和策略跑动顺序无关。

对应实现是 `_trim_to_capacity_round_robin()`，测试见 `tests/test_screening_watchlist.py`
里 `test_capacity_round_robin_*` 五个用例。
