...
- [修复] 新增 `scripts/repair_watchlist_hit_count.py`，按 `data/watchlist/history/` 重放并重算 `hit_count`。同一天去重是 2026-08-30（46a0470）才加上的，此前 `merge_run()` 每调用一次就 +1，同一天手动重跑几轮 weekly 会把计数灌高；已写进 `current.json` 的数据不会自愈，因为 `merge_run` 只做增量。实测 8 只票存储 6/7 而真实值为 2，全部顶到命中加分上限 +10，通过 `rank_score()` 影响行业配额与容量裁剪（构造用例中一只 70 分的虚高票能挤掉 77 分的干净票）；另有条目因 TTL 剪枝后重建而低于真实值，同样修正。脚本幂等，`--dry-run` 只报告差异，history 中查不到的条目保持原值以免误伤 pinned 占位。
- [修复] 观察名单的容量裁剪改为**桶内按策略轮转**取票，不再按 `latest_score` 全桶排序。`latest_score` 是该策略硬筛存活池内的分位排名，池子大小差一个数量级时跨策略不可比：进攻桶实测 `volume_breakout` 存活池 19 只（分数 70.13~82.22）、`capital_heat` 93 只（68.93~73.84）、`theme_momentum` 128 只（67.98~69.40），后者的最高分低于前者的最低分，15 个名额里 13 个被 `volume_breakout` 占据，`theme_momentum` 独立席位为 0。分桶此前只解决了跨桶不可比，桶内跨策略仍在拿不可比的分数排序。轮转让各策略按自身分数排队后轮流取票，同一批历史数据回放下进攻桶从 10/6/4 变成 5/8/7。相比固定席位，策略增删时自动适配、候选不足时名额流向其他策略而非空置。多策略共同命中的票优先入选（只占一个名额，记在主策略名下），轮转顺序按策略名固定以保证结果确定。
